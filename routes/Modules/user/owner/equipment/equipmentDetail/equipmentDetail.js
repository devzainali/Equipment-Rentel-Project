...
router.get('/', async (req, res) => {
    ...
    initializegetTopRated.then(function (IDs) {
        ...
        Equipment.findAndCountAll({
            ...
            include: [{
                model: User, attributes: ["id", "firstName", "lastName", "companyName", "address", "image",], include: [{
                    model: Rating, include: [{
                        model: User, attributes: ["id", "firstName", "lastName", "image"]
                    }]
                }, {
                    model: Equipment, include: [{ model: Rating, attributes: ["equipmentStar",] }], where: {
                        id: { [Op.notIn]: BookObject }, active: 0,
